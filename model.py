...
class StyleTransferModel:
    ...
    def transfer_style(self, content_img_stream, style_img_stream):
        ...
        output = self.run_style_transfer(self.cnn, self.cnn_normalization_mean,
                                         self.cnn_normalization_std,
                                    content_img, style_img, input_img)


        print("Output.shape: ", output.shape, output.detach().numpy()[0].shape)
        print("output.type: ", type(output), type(output.detach()[0]))
    ...
    def run_style_transfer(self, cnn, normalization_mean, normalization_std,
        ...
        while run[0] <= num_steps:
            ...
            def closure():
                # correct the values, not letting them get away from [0;1] range
    ...
    def process_image(self, img_stream):
        ## TOOD image size, device and transforms do not change during the whole model processing,
        # so we need to get everything to constructor
        imsize = 256

        loader = transforms.Compose([
            transforms.Resize(imsize),  # normalizing input size
            transforms.CenterCrop(imsize),
            transforms.ToTensor()])  # convinient format

        image = Image.open(img_stream)
        image = loader(image).unsqueeze(0)
        return image#(torch.float)
